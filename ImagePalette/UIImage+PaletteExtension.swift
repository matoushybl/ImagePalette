...
extension UIImage {
	...
	*/
	internal func scaleDown(_ targetMaxDimension: CGFloat) -> UIImage? {
		let size = self.size
	...
	internal var pixels: Array<Int64> {
        ...
        if let context = CGContext(data: bitmapData, width: pixelsWide, height: pixelsHigh, bitsPerComponent: 8, bytesPerRow: bitmapBytesPerRow, space: colorSpace, bitmapInfo: CGImageAlphaInfo.premultipliedFirst.rawValue) {
            context.draw(image, in: CGRect(x: 0.0, y: 0.0, width: CGFloat(pixelsWide), height: CGFloat(pixelsHigh)))

            var pixels = Array<Int64>()
            let unconstrainedData = context.data
            if let data = UnsafePointer<UInt8>(unconstrainedData?.assumingMemoryBound(to: UInt8.IntegerLiteralType.self)) {
